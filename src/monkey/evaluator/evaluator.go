...
/*
Function that routes to proper handling function for each supported infix operator
*/
func evalInfixExpression(
	...
	switch {
	// int + int, int - int, etc.
	// this check should go before the bool operands so that value comparison is done
	case left.Type() == object.INTEGER_OBJ && right.Type() == object.INTEGER_OBJ:
		return evalIntegerInfixExpression(operator, left, right)

	// string + string, i.e. concatentation
	case left.Type() == object.STRING_OBJ && right.Type() == object.STRING_OBJ:
		return evalStringInfixExpression(operator, left, right)

	// bool == bool, bool != bool
	// we're doing point comparison here because bools point to either one of the two
	case operator == "==":
...
/*
Evaluate if-else block statement depending on the Condition
*/
...
func applyFunction(fn object.Object, args []object.Object) object.Object {
	switch fn := fn.(type) {
	case *object.Function: // user-defined functions
		extendedEnv := extendFunctionEnv(fn, args)
		evaluated := Eval(fn.Body, extendedEnv)
		return unwrapReturnValue(evaluated)

	case *object.Builtin: // built-in functions
		return fn.Fn(args...)
